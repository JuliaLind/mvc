...
Så länge som en card ligger i en DeckOfCards så är relationen mellan Card och DeckOfCards är en komposition - korten skapas inuti instansen av DeckOfCards och om vi tar bort DeckOfCards så kommer även tillhörande instanser av Card försvinna. När vi plockar ut kortet från DeckOfCard med draw metoden så kan kortet fortsätta att existera även om DeckOfCards skulle raderas, så här blir jag osäker på vilken relation som gäller. Jag skulle kanske vilja sälga aggregation? 

Relationen mellan Card och CardHand var också lite svår att avgöra om den skulle definieras som aggregation eller komposition. En hand skapar ju inte kort själv utan får dessa från en kortlek. Innan korten lagts till i CardHand så är den tom. Så card kan finnas utan hand - men däremot så snart korten plockas ut från DeckOfCards och skickas in i CardHand så är kortens existens beroende av att CardHand fortsätter att existera.  

Relationen mellan DeckOfCards och CardHand är association. En CardHand får/tar kort från DeckOfCards. CardHand kommer inte att försvinna om vi tar bort DeckOfCards och vice versa, och det finns inte heller en ägande-relation åt något av hållen.
